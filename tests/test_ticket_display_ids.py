...
class TicketDisplayIDsTest(unittest.TestCase):
    ...
    def create(self, ticket_id):
        return tickets.create_ticket(self.root, actor="sol-master", ticket_id=ticket_id, title=ticket_id)
    ...
    def test_backfill_is_idempotent_preserves_history_and_references(self):
        ...
        tickets.add_dependency(self.root, "legacy-child", actor="sol-master", dep_type="BLOCKED_BY", target="legacy-parent")
    ...
    def test_archive_and_cache_loss_do_not_renumber_or_reuse(self):
        first = self.create("Z")
        tickets.transition_ticket(self.root, "Z", actor="sol-master", stage="CANCELLED")
        tickets.archive_ticket(self.root, "Z", actor="sol-master")
    ...
    def test_project_prefix_freezes_and_namespaces_are_independent(self):
        ...
        self.assertEqual(tickets.create_ticket(other, actor="sol-master", ticket_id="A", title="A")["display_id"], "ATLAS-1")
    ...
    def test_migration_notifies_pollers_and_includes_archived_tickets(self):
        self.create("old")
        tickets.transition_ticket(self.root, "old", actor="sol-master", stage="CANCELLED")
        tickets.archive_ticket(self.root, "old", actor="sol-master")
